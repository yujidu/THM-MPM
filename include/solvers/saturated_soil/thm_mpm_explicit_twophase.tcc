...
mpm::THMMPMExplicitTwoPhase<Tdim>::THMMPMExplicitTwoPhase(
    const std::shared_ptr<IO>& io)
    : mpm::MPMBase<Tdim>(io) {
  //! Logger
  console_ = spdlog::get("THMMPMExplicitTwoPhase");
}
...
template <unsigned Tdim>
void mpm::THMMPMExplicitTwoPhase<Tdim>::compute_stress_strain() {
...
template <unsigned Tdim>
void mpm::THMMPMExplicitTwoPhase<Tdim>::pressure_smoothing(unsigned phase) {
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::solve() {
  bool status = true;
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::pre_process() {
  bool status = true;
  console_->info("MPM analysis type {}", io_->analysis_type());
...
// Main loop
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::get_deformation_task() {
  bool status = true;
  if (step_ < nsteps_) {
...
template <unsigned Tdim>
void mpm::THMMPMExplicitTwoPhase<Tdim>::get_info(unsigned& dim, bool& resume,
...
template <unsigned Tdim>
void mpm::THMMPMExplicitTwoPhase<Tdim>::get_status(double& dt, unsigned& step,
                                        unsigned& nsteps,
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::send_deformation_task(
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::set_stress_task(const Eigen::MatrixXd& stresses,
                                             bool increment) {
  bool status = true;
  if (step_ < nsteps_) {
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::set_porosity_task(
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::set_fabric_task(std::string fabric_type,
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::set_rotation_task(
...
template <unsigned Tdim>
bool mpm::THMMPMExplicitTwoPhase<Tdim>::update_state_task() {
  bool status = true;
  if (step_ < nsteps_) {
