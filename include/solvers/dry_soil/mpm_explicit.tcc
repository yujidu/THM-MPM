...
bool mpm::MPMExplicit<Tdim>::solve() {
  ...
  // Variable timestep
  if (analysis_.find("char_size") != analysis_.end())
    char_size_ = analysis_["char_size"].template get<double>();

  // Variable timestep
  if (analysis_.find("para_m") != analysis_.end())
    para_m_ = analysis_["para_m"].template get<double>();
  ...
  for (step_ = 0; step_ <= nsteps_; ++step_) {
    ...
    mesh_->compute_particle_nonlocal_variable(char_size_, para_m_);
