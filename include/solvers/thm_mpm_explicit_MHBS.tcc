...
bool mpm::THMMPMExplicitMHBS<Tdim>::solve() {
  ...
  mesh_->iterate_over_cells(std::bind(
    ...
    if (current_time_<= 10.) 
      gravity = this->gravity_ * current_time_ / 10.;
...
void mpm::THMMPMExplicitMHBS<Tdim>::compute_critical_timestep_size(double dt) {
  ...
  double hydrate_density = materials->template property<double>(std::string("hydrate_density"));
  double hydrate_specific_heat = materials->template property<double>(std::string("hydrate_specific_heat"));
  double hydrate_thermal_conductivity = materials->template property<double>(std::string("hydrate_thermal_conductivity"));
  ...
  double density_mixture3 = (1 - porosity) * density + porosity * hydrate_density;
  ...
  console_->info("Critical time step size for elastic wave propagation (hydrate base) is {} s", critical_dt13);  
  ...
  // Compute timestep for heat transfer eqaution - pure hydrate
  double k_mixture2 = (1 - porosity) * thermal_conductivity + porosity * hydrate_thermal_conductivity;
  double c_mixture2 = (1 - porosity) * density * specific_heat + porosity * hydrate_density * hydrate_specific_heat;
  double critical_dt22 = cellsize_min * cellsize_min * c_mixture2 / k_mixture2;                                
  console_->info("Critical time step size for thermal conduction equation (hydrate base) is {} s", critical_dt22);
...
bool mpm::THMMPMExplicitMHBS<Tdim>::send_saturation_task(
  ...
  bool status = true;
  mesh_->get_hydrate_saturation(id, hydrate_saturation);
  return status;
}
...
/////////////////////////////////////////////////////////////////////////////////
    ...
    // if (current_time_<= 10) {
    //   this->gravity_(1) = -9.81 * current_time_;
    // }
    // else this->gravity_(1) = -9.81;
