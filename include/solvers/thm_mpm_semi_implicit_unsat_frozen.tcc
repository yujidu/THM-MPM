...
mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::THMMPMSemiImplicitUnsatFrozen(
    const std::shared_ptr<IO>& io)
    : mpm::MPMBase<Tdim>(io) {
  //! Logger
  console_ = spdlog::get("THMMPMSemiImplicitUnsatFrozen");
}

//! THM-MPM semi-implicit two phase with phase change solver
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::solve() {
  bool status = true;
...
////////////////////////////////////////////////////////////////////////////////
    ...
    if (this->stress_update_ == mpm::StressUpdate::USF) {
      ...
      mesh_->iterate_over_particles(
        std::bind(&mpm::ParticleBase<Tdim>::update_gas_saturation,
                  std::placeholders::_1, dt_));

      // Iterate over each particle to update gas saturation
      mesh_->iterate_over_particles(
        std::bind(&mpm::ParticleBase<Tdim>::compute_gas_saturation_smoothing,
                  std::placeholders::_1));                      
...
// Initialise matrix
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::initialise_matrix() {
  bool status = true;
  try {
    // Max iteration steps
    unsigned max_iter =
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::reinitialise_matrix() {
  bool status = true;
  try {
...
template <unsigned Tdim>
void mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::pressure_smoothing(unsigned phase) {
...
//! Compute intermediate velocity
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::compute_intermediate_velocity(
    std::string solver_type) {
  bool status = true;
  try {
    // Map K_inter to cell
...
// Compute poisson equation
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::compute_poisson_equation(
    std::string solver_type) {
  bool status = true;
  try {
...
//! Compute corrected force
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::compute_corrected_force() {
  bool status = true;
  try {
...
template <unsigned Tdim>
void mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::compute_critical_timestep_size(double dt) {
  const unsigned soil_skeleton = mpm::ParticlePhase::Solid;
  const unsigned pore_liquid = mpm::ParticlePhase::Liquid;
  const unsigned mixture = mpm::ParticlePhase::Mixture;
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::pre_process() {
  bool status = true;
  console_->info("MPM analysis type {}", io_->analysis_type());
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::get_deformation_task() {
  bool status = true;
...
template <unsigned Tdim>
void mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::get_info(unsigned& dim, bool& resume,
...
template <unsigned Tdim>
void mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::get_status(double& dt, unsigned& step,
                                                    unsigned& nsteps,
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::send_deformation_task(
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::send_temperature_task(
    std::vector<unsigned>& id,
...
// Set particle stress
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::set_stress_task(
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::set_porosity_task(
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::set_fabric_task(
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::set_rotation_task(
...
template <unsigned Tdim>
bool mpm::THMMPMSemiImplicitUnsatFrozen<Tdim>::update_state_task() {
  // Two phases and its mixture (soil skeleton and pore liquid)
  // NOTE: Mixture nodal variables are stored at the same memory index as the
  // solid phase
...
// /////////////////////////////////////////////////////////////////////////////////
    ...
    if (step_ == nsteps_) {
      auto solver_end = std::chrono::steady_clock::now();
      console_->info(
          "Rank {}, THMMPMSemiImplicitUnsatFrozen {} solver duration: {} ms", 0,
          (this->stress_update_ == mpm::StressUpdate::USL ? "USL" : "USF"),
          std::chrono::duration_cast<std::chrono::milliseconds>(solver_end -
                                                                solver_begin)
              .count());
