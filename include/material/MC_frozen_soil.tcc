...
mpm::MohrCoulomb_FrozenSoil<Tdim>::MohrCoulomb_FrozenSoil(unsigned id,
  ...
  try {
    ...
    phi_residual_ =
        material_properties.at("residual_friction").template get<double>() * M_PI / 180.;
    psi_residual_ =
        material_properties.at("residual_dilation").template get<double>() * M_PI / 180.;
    cohesion_residual_ =
        material_properties.at("residual_cohesion").template get<double>();
    // Frozen friction, dilation and cohesion
    dphi_max_ =
        material_properties.at("dfriction_max").template get<double>() * M_PI / 180.;
    dcohesion_max_ = material_properties.at("dcohesion_max").template get<double>();
    // Get lamda    
    ...
    eta_c_ = material_properties.at("eta_c").template get<double>(); 
    sf_ = material_properties.at("sf").template get<double>();     
    // Properties
...
template <unsigned Tdim>
mpm::dense_map mpm::MohrCoulomb_FrozenSoil<Tdim>::initialise_state_variables() {
...
template <unsigned Tdim>
bool mpm::MohrCoulomb_FrozenSoil<Tdim>::compute_elastic_tensor() {
  // Shear modulus
...
template <unsigned Tdim>
bool mpm::MohrCoulomb_FrozenSoil<Tdim>::compute_stress_invariants(
...
typename mpm::mohrcoulomb::FailureState
    mpm::MohrCoulomb_FrozenSoil<Tdim>::compute_yield_state(
...
template <unsigned Tdim>
void mpm::MohrCoulomb_FrozenSoil<Tdim>::compute_df_dp(
  ...
  // Compute dp/dsigma and dp/dj in shear yield
    ...
    const double xi = 0.1;
    double omega = std::pow((xi * cohesion_ * tan(psi)), 2) +
  ...
  (*softening) = 0.;
  if (softening_ && pdstrain > 0) {
    if (softening_type_ == "model_4") {
      double liquid_saturation = ptr->liquid_saturation();
      double cryo_suction = std::pow(
                            (std::pow(liquid_saturation, -1 / lambda_) - 1),
                            (1 - lambda_));
      dphi_dpstrain = 0.;
      dc_dpstrain = 0.;
      if (cryo_suction > 0) {
        // Compute dPhi/dPstrain
        dphi_dpstrain = -eta_s_ * (cohesion_peak_ - cohesion_residual_) * 
                          (1 - std::exp(-eta_c_ * cryo_suction)) *
                          std::exp(-eta_s_ * pdstrain);
        // Compute dc/dPstrain
        dc_dpstrain = -eta_s_ * (phi_peak_ - phi_residual_) *
                          (1 - std::exp(-eta_c_ * cryo_suction)) *
                          std::exp(-eta_s_ * pdstrain);
      }
    } else {
...
//! Compute stress
template <unsigned Tdim>
Eigen::Matrix<double, 6, 1> mpm::MohrCoulomb_FrozenSoil<Tdim>::compute_stress(
  ...
  if (softening_) {
    ...
    (*state_vars)["cohesion"] = cohesion_residual_ + (cohesion_peak_ - cohesion_residual_) *
                            std::exp(-eta_s_ * pdstrain);
    (*state_vars)["psi"] = psi_peak_ * std::exp(-sf_ * pdstrain);
    
    // loss of ice cementation
    if (softening_type_ == "model_1" || softening_type_ == "model_2") {
      double pore_liquid_pressure = ptr->pore_pressure();
      double pore_ice_pressure = ptr->pore_ice_pressure();
      double cryo_suction = (pore_ice_pressure - pore_liquid_pressure) / p0_;
      if (cryo_suction > 0) {
        (*state_vars)["cohesion"] += dcohesion_max_ * 
                              (1 - std::exp(-eta_c_ * cryo_suction));
        (*state_vars)["phi"] += dphi_max_ *
                              (1 - std::exp(-eta_c_ * cryo_suction));
      }                                                        
    } 
    else if (softening_type_ == "model_3") {
      double liquid_saturation = ptr->liquid_saturation();
      double cryo_suction = std::pow((std::pow(liquid_saturation, -1 / lambda_) - 1), (1 - lambda_));      
      if (cryo_suction > 0) {
        (*state_vars)["cohesion"] += dcohesion_max_ * 
                              (1 - std::exp(-eta_c_ * cryo_suction));
        (*state_vars)["phi"] += dphi_max_ *
    ...
    else if (softening_type_ == "model_4") {
      double liquid_saturation = ptr->liquid_saturation();
      double cryo_suction = std::pow((std::pow(liquid_saturation, -1 / lambda_) - 1), (1 - lambda_));      
      if (cryo_suction > 0) {
