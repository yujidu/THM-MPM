...
mpm::LinearElastic<Tdim>::LinearElastic(unsigned id,
  ...
  try {
    ...
    // ice_youngs_modulus_ =
    //     material_properties.at("ice_youngs_modulus").template get<double>();         
...
Eigen::Matrix<double, 6, 1> mpm::LinearElastic<Tdim>::compute_stress(
  ...
  // double youngs_modulus = youngs_modulus_ + 6E9 * 1 * Sh;
  // bulk_modulus_ = youngs_modulus / (3.0 * (1. - 2. * poisson_ratio_));

  // // Shear modulus
  // const double G = youngs_modulus / (2.0 * (1. + poisson_ratio_));
