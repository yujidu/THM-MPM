...
Eigen::Matrix<double, 6, 1> mpm::MohrCoulomb<Tdim>::compute_stress(
  ...
  // Get equivalent plastic deviatoric strain
  // (*state_vars).at("pdstrain") = ptr->damage_variable();
  const double pdstrain = (*state_vars).at("pdstrain");
  ...
  if (yield_type_trial == mpm::mohrcoulomb::FailureState::Elastic) {
    // double omega_new = 0.9 * std::exp(-A_s_ * pdstrain) + 0.1;
    // trial_stress *= omega_new;
    return trial_stress;
  }
  ...
  this->compute_stress_invariants(stress_new , state_vars);
  ...
  this->compute_df_dp(yield_type, state_vars, stress_new, &df_dsigma, &dp_dsigma,
                      &dp_dq, &softening, ptr);
  const double lambda =
  ...
  // double omega_new = 0.9 * std::exp(-A_s_ * (*state_vars).at("pdstrain")) + 0.1;
