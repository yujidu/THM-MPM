...
inline Eigen::VectorXd
  ...
  try {
    ...
    for (unsigned n = 0; n < Tnfunctions; ++n) {
      ...
      Eigen::Matrix<double, Tdim, 1> natural_xi = particle_size;
      ...
      for (unsigned i = 0; i < Tdim; ++i) {
        ...
        double r = npni / element_length;

        double left_boundary = 0.;
        double right_boundary = 26.;
        double bottom_boundary = 0.;
        double cell_size = 0.4;

        bool left_cell, right_cell, bottom_cell, inner_cell;
        left_cell = false;
        right_cell = false;
        bottom_cell = false;
        inner_cell = false;
        ...
        if (i == 0){
          if (((natural_xi(0) - left_boundary) >= 0.) &&
              ((natural_xi(0) - left_boundary) <= cell_size)) {
              left_cell = true;
          } 
          else if (((natural_xi(0) - right_boundary) >= (-1. * cell_size)) &&
                  ((natural_xi(0) - right_boundary) <= 0.)) {
              right_cell = true;
          }
          else if (((natural_xi(0) - left_boundary) >= cell_size) && 
                  ((natural_xi(0) - right_boundary) <= (-1. * cell_size))) {
              inner_cell = true;
          } else {
            std::cout << "BS shapefn 0: Point location outside area of influence" 
                      << " p coorsinate = " << natural_xi(0) << "\t" << natural_xi(1) << std::endl;
          }
        } 

        if (i == 1){
          if (((natural_xi(1) - bottom_boundary) >= 0.) &&
              ((natural_xi(1) - bottom_boundary) <= cell_size)) {
              bottom_cell = true;
          } 
          else if ((natural_xi(1) - bottom_boundary) >= cell_size) {
              inner_cell = true;
          } else {
            std::cout << "BS shapefn 0: Point location outside area of influence" 
                      << " p coorsinate = " << natural_xi(0) << "\t" << natural_xi(1) << std::endl;
        ...
        if (inner_cell) {
          if ((r >= -2) && (r <= -1)) {
            sni(i) = 1./6. * std::pow(r, 3) + std::pow(r, 2) + 2. * r + 4./3.;
          } else if ((r >= -1) && (r <= 0)) {
            sni(i) = -1./2. * std::pow(r, 3) - std::pow(r, 2) + 2./3.;
          } else if ((r >= 0) && (r <= 1)) {
            sni(i) = 1./2. * std::pow(r, 3) - std::pow(r, 2) + 2./3.;
          } else if ((r >= 1) && (r <= 2)) {
            sni(i) = -1./6. * std::pow(r, 3) + std::pow(r, 2) - 2. * r + 4./3.;
          ...
          throw std::runtime_error(
              "BS shapefn 2: Point location outside area of influence");
          }
        } 
        // First cell to the left/bottom boundary 
        else if (left_cell || bottom_cell) {
          if ((r >= 1) && (r <= 2)) {
            sni(i) = 0; // sure
          } else if ((r >= 0) && (r <= 1)) {
            sni(i) = 1./6. * std::pow(r, 3) - r + 1.; // sure
          } else if ((r >= -1) && (r <= 0)) {
            sni(i) = -1./3. * std::pow(r, 3) - std::pow(r, 2) + 2./3.; // sure
          } else if ((r >= -2) && (r <= -1)) {
            sni(i) = 1./6. * std::pow(r, 3) + std::pow(r, 2) + 2. * r + 4./3.; // sure
          } else {
          throw std::runtime_error(
              "BS shapefn 3: Point location outside area of influence");
          }
        }
        // First cell to the right boundary 
        else if (right_cell) {
          if ((r >= -2) && (r <= -1)) {
            sni(i) = 0; // sure
          } else if ((r >= -1) && (r <= 0)) {
          ...
          throw std::runtime_error(
              "BS shapefn 4: Point location outside area of influence");
            }
        } else {
          throw std::runtime_error(
              "BS shapefn 5: Point location outside area of influence");
        }
    ...
    for (unsigned n = 0; n < Tnfunctions; ++n) {
      ...
      for (unsigned i = 0; i < Tdim; ++i) {
        ...
        if (i == 0){
          ...
          else if (((natural_xi(0) - right_boundary) >= -cell_size) &&
                  ((natural_xi(0) - right_boundary) <= 0.)) {
              right_cell = true;
          }
          else if (((natural_xi(0) - left_boundary) >= cell_size) && 
                  ((natural_xi(0) - right_boundary) <= -cell_size)) {
              inner_cell = true;
          } else {
          throw std::runtime_error(
              "BS grad shapefn 0: Point location outside area of influence");
          }
        } 
        ...
        if (i == 1){
          ...
          throw std::runtime_error(
              "BS grad shapefn 1: Point location outside area of influence");
        }
        }
        // case3 = true;
        ...
        if (inner_cell) {
          if ((r >= -2) && (r <= -1)) {
            sni(i) = 1./6. * std::pow(r, 3) + std::pow(r, 2) + 2. * r + 4./3.;
            dni(i) = 1./2. * std::pow(r, 2) + 2. * r + 2.;
          } else if ((r >= -1) && (r <= 0)) {
            sni(i) = -1./2. * std::pow(r, 3) - std::pow(r, 2) + 2./3.;
            dni(i) = -3./2. * std::pow(r, 2) - 2. * r;
          } else if ((r >= 0) && (r <= 1)) {
            sni(i) = 1./2. * std::pow(r, 3) - std::pow(r, 2) + 2./3.;
            dni(i) = 3./2. * std::pow(r, 2) - 2. * r;
          } else if ((r >= 1) && (r <= 2)) {
          ...
          throw std::runtime_error(
              "BS grad shapefn 2: Point location outside area of influence");
          }
        } 
        // First cell to the left/bottom boundary 
        else if (left_cell || bottom_cell) {
          if ((r >= 1) && (r <= 2)) {
            ...
            sni(i) = 1./6. * std::pow(r, 3) - r + 1.; // sure
            dni(i) = 1./2. * std::pow(r, 2) - 1.;
          } else if ((r >= -1) && (r <= 0)) {
            sni(i) = -1./3. * std::pow(r, 3) - std::pow(r, 2) + 2./3.; // sure
            dni(i) = -1. * std::pow(r, 2) - 2. * r; 
          } else if ((r >= -2) && (r <= -1)) {
            sni(i) = 1./6. * std::pow(r, 3) + std::pow(r, 2) + 2. * r + 4./3.; // sure
            dni(i) = 1./2. * std::pow(r, 2) + 2. * r + 2.;
          } else {
          throw std::runtime_error(
              "BS grad shapefn 3: Point location outside area of influence");
          }
        }
        // First cell to the right boundary 
        else if (right_cell) {
          if ((r >= -2) && (r <= -1)) {
            ...
            sni(i) = 1./3. * std::pow(r, 3) - std::pow(r, 2) + 2./3.; // sure
            dni(i) = std::pow(r, 2) - 2. * r; 
          } else if ((r >= 1) && (r <= 2)) {
            sni(i) = -1./6. * std::pow(r, 3) + std::pow(r, 2) - 2. * r + 4./3.; //sure
            dni(i) = -1./2. * std::pow(r, 2) + 2. * r - 2.;
          } else {
          throw std::runtime_error(
              "BS grad shapefn 4: Point location outside area of influence");
            }
        } else {
          throw std::runtime_error(
              "BS grad shapefn 5: Point location outside area of influence");
        }
