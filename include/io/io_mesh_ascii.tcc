...
//! Read temperatures of particles
template <unsigned Tdim>
std::vector<double> mpm::IOMeshAscii<Tdim>::read_particles_temperatures(
    const std::string& particles_temperatures) {
  ...
  // input file stream
  std::fstream file;
  file.open(particles_temperatures.c_str(), std::ios::in);
  ...
  try {
        ...
        if ((line.find('#') == std::string::npos) &&
          ...
          while (istream.good()) {
            if (!read_first_line) {
              // Read number of nodes and cells
              istream >> nparticles;
              temperatures.reserve(nparticles);
              read_first_line = true;
              break;
            }
            // temperature
    ...
    console_->error("Read particle temperature: {}", exception.what());
    file.close();
  }
  return temperatures;
}
...
//! Read volumes of particles
template <unsigned Tdim>
std::vector<double> mpm::IOMeshAscii<Tdim>::read_particles_volumes(
    const std::string& particles_volumes) {
  ...
  // input file stream
  std::fstream file;
  file.open(particles_volumes.c_str(), std::ios::in);
  ...
  try {
    ...
    console_->error("Read particle volume: {}", exception.what());
    file.close();
  }
  return volumes;
}
