...
class VtkWriter {
 public:
  ...
  void create_new_dataset(){
    ...
    pdata_ = vtkSmartPointer<vtkPolyData>::New();
  ...
  void write_scalar_point_data(const std::vector<double>& data,
                                const std::string& data_field);
  //! Write vector data
  //! \param[in] data Vector data
  //! \param[in] data_field Field name ("Displacement", "Forces")
  void write_vector_point_data(const std::vector<Eigen::Vector3d>& data,
                                const std::string& data_fields);

  //! Write tensor data
  //! \param[in] data Tensor data
  //! \param[in] data_field Field name ("Stresses", "Strains")
  void write_tensor_vector_point_data(
 ...
 private:
  ...
  //! vtk of all data
  vtkSmartPointer<vtkPolyData> pdata_;
};
