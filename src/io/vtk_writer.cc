...
void VtkWriter::write_file(const std::string& filename) {
  auto writer = vtkSmartPointer<vtkXMLPolyDataWriter>::New();
  writer->SetFileName(filename.c_str());
  writer->SetDataModeToBinary();

#if VTK_MAJOR_VERSION <= 5
  writer->SetInput(pdata_);
#else
  writer->SetInputData(pdata_);
#endif
...
void VtkWriter::write_vector_point_data(
  ...
  vectordata->SetNumberOfTuples(pdata_->GetNumberOfPoints());
  ...
  // Add the SignedDistances to the grid
  pdata_->GetPointData()->AddArray(vectordata);
}

//! Write scalar data
void VtkWriter::write_scalar_point_data(const std::vector<double>& data,
                                        const std::string& data_field) {
  ...
  pdata_->GetPointData()->AddArray(scalardata);
}
